...
// Function to search and count pattern occurrences
        ...
        while (temp != root) {
            if (temp->pattern_count > 0) {
                for (int j = 0; j < temp->pattern_count; j++) {
                    // Need to ensure atomic updates to avoid race conditions
                    #pragma omp atomic
...
int main(int ac, char** av) {
    ...
    #pragma omp parallel
    {
        // N eed to ensure only one thread manages traversal
        #pragma omp single
